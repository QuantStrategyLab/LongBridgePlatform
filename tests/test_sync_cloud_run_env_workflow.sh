...
grep -Fq 'LONGBRIDGE_FRACTIONAL_LIMIT_BUY_FALLBACK_TO_MARKET: ${{ vars.LONGBRIDGE_FRACTIONAL_LIMIT_BUY_FALLBACK_TO_MARKET }}' "$workflow_file"
grep -Fq 'RUNTIME_TARGET_JSON: ${{ vars.RUNTIME_TARGET_JSON }}' "$workflow_file"
grep -Fq "ACCOUNT_REGION: \${{ vars.ACCOUNT_REGION || 'PAPER' }}" "$workflow_file"
grep -Fq "ACCOUNT_REGION: \${{ vars.ACCOUNT_REGION || 'SG' }}" "$workflow_file"
grep -Fq 'echo "enabled=false" >> "$GITHUB_OUTPUT"' "$workflow_file"
grep -Fq "Skipping PAPER Cloud Run env sync because ENABLE_GITHUB_ENV_SYNC is not set to true." "$workflow_file"
grep -Fq "Skipping SG Cloud Run env sync because ENABLE_GITHUB_ENV_SYNC is not set to true." "$workflow_file"
grep -Fq "PAPER Cloud Run env sync is enabled, but these values are missing:" "$workflow_file"
grep -Fq "SG Cloud Run env sync is enabled, but these values are missing:" "$workflow_file"
grep -Fq "set CLOUD_RUN_REGION on the longbridge-paper Environment" "$workflow_file"
grep -Fq "set CLOUD_RUN_REGION on the longbridge-sg Environment" "$workflow_file"
grep -Fq "Set LONGPORT_APP_KEY_SECRET_NAME and LONGPORT_APP_SECRET_SECRET_NAME on the longbridge-paper Environment" "$workflow_file"
...
grep -Fq 'LONGBRIDGE_FRACTIONAL_LIMIT_BUY_FALLBACK_TO_MARKET=${LONGBRIDGE_FRACTIONAL_LIMIT_BUY_FALLBACK_TO_MARKET}' "$workflow_file"
