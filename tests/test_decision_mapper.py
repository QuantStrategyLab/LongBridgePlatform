...
class DecisionMapperTests(unittest.TestCase):
    def test_maps_semiconductor_strategy_decision_to_execution_plan(self):
        ...
        self.assertEqual(plan["strategy_assets"], ("SOXL", "SOXX", "BOXX", "QQQI", "SPYI"))
        self.assertEqual(plan["limit_order_symbols"], ("SOXL", "SOXX", "QQQI", "SPYI"))
        self.assertEqual(plan["portfolio_rows"], (("SOXL", "SOXX"), ("QQQI", "SPYI"), ("BOXX",)))
        self.assertEqual(plan["targets"]["BOXX"], 15000.0)
        self.assertEqual(plan["threshold_value"], 500.0)
